...
for channel in ["emuHighMT", "OffZHighMET"]:
	...
	L_cuts = np.zeros((len(minDRJetToLep3_cuts), len(minDRJetToLep4_cuts), len(njets_cuts)))
	for idx1 in range(len(minDRJetToLep3_cuts)):
		for idx2 in range(len(minDRJetToLep4_cuts)):
			for idx3 in range(len(njets_cuts)):
				cut_this = "(minDRJetToLep3>"+str(minDRJetToLep3_cuts[idx1])+" || minDRJetToLep3 < 0) && (minDRJetToLep4>"+str(minDRJetToLep4_cuts[idx2])+" || minDRJetToLep4 < 0) && nj<"+str(njets_cuts[idx3]) 
				#print(cut_this)
				...
				print(str(minDRJetToLep3_cuts[idx1])+", "+str(minDRJetToLep4_cuts[idx2]) + ", "+ str(njets_cuts[idx3])+ ", "+str(s) +", "+str(b)+ ", "+ str(L))
				L_cuts[idx1][idx2][idx3] = L
	print("summary=================")
	print(L_cuts)
